import React, { useEffect, useState } from 'react'
import Table from '@mui/material/Table';
import TableBody from '@mui/material/TableBody';
import TableCell from '@mui/material/TableCell';
import TableContainer from '@mui/material/TableContainer';
import TableHead from '@mui/material/TableHead';
import TableRow from '@mui/material/TableRow';
import Paper from '@mui/material/Paper';
import { Box, FormControl, Grid, InputLabel, NativeSelect, Typography } from '@mui/material';
import BarChart from '../Charts/BarChart';

export default function RaceDetail({ year, title, detail, data, setDetail, arrDetail }) {
    let arrTemp = data.toSorted((a, b) => {
        let fa = a.driver,
            fb = b.driver;
        if (fa < fb) {
            return -1;
        }
        if (fa > fb) {
            return 1;
        }
        return 0;
    });
    const [chart, setChart] = useState({
        labels: arrTemp.map((item) => item.driver),
        datasets: [
            {
                label: `${year} ${title.slice(0, -1)} RESULTS: ${detail.toUpperCase()} GRAND PRIX`,
                data: arrTemp.map((item) => item.pts),
                backgroundColor: [
                    '#e10600',
                    '#05060c'
                ],
                hoverOffset: 4
            },
        ],
    })
    const [detailData, setDetailData] = useState(detail);

    useEffect(() => {
        setChart({
            labels: arrTemp.map((item) => item.driver),
            datasets: [
                {
                    label: `${year} ${title.slice(0, -1)} RESULTS: ${detail.toUpperCase()} GRAND PRIX`,
                    data: arrTemp.map((item) => item.pts),
                    backgroundColor: [
                        '#e10600',
                        '#05060c'
                    ],
                    hoverOffset: 4
                },
            ],
        })
        setDetailData(detail)
    }, [year, detail, data])

    return (
        <>
            <Box sx={{ flexGrow: 1 }}>
                <Grid container spacing={3}>
                    <Grid item xs></Grid>
                    <Grid item xs={10}>
                        <Typography variant='h4' marginBottom='10px' fontFamily='Orbitron' sx={{ float: 'left' }}>
                            {year} {title.slice(0, -1)} RACE RESULTS: {detail.toUpperCase()} GRAND PRIX
                        </Typography>
                    </Grid>
                    <Grid item xs></Grid>
                </Grid>
                <Grid container spacing={3}>
                    <Grid item xs></Grid>
                    <Grid item xs={10}>
                        <TableContainer component={Paper}>
                            <Table sx={{ minWidth: 650 }} aria-label="simple table">
                                <TableHead>
                                    <TableRow style={{ backgroundColor: '#e10600' }}>
                                        <TableCell style={{ color: '#ffffff' }}>POS</TableCell>
                                        <TableCell align="left" style={{ color: '#ffffff' }}>NO</TableCell>
                                        <TableCell align="left" style={{ color: '#ffffff' }}>DRIVER&nbsp;</TableCell>
                                        <TableCell align="left" style={{ color: '#ffffff' }}>CAR&nbsp;</TableCell>
                                        <TableCell align="left" style={{ color: '#ffffff' }}>LAPS&nbsp;</TableCell>
                                        <TableCell align="left" style={{ color: '#ffffff' }}>TIME/RETIRED&nbsp;</TableCell>
                                        <TableCell align="left" style={{ color: '#ffffff' }}>PTS&nbsp;</TableCell>
                                    </TableRow>
                                </TableHead>
                                <TableBody>
                                    {
                                        data.map((row, index) => (
                                            <TableRow
                                                hover
                                                key={index}
                                                sx={{ '&:last-child td, &:last-child th': { border: 0 } }}
                                            >
                                                <TableCell component="th" scope="row">
                                                    {row.pos}
                                                </TableCell>
                                                <TableCell align="left">{row.no}</TableCell>
                                                <TableCell align="left">{row.driver}</TableCell>
                                                <TableCell align="left">{row.car}</TableCell>
                                                <TableCell align="left">{row.laps}</TableCell>
                                                <TableCell align="left">{row.time}</TableCell>
                                                <TableCell align="left">{row.pts}</TableCell>
                                            </TableRow>
                                        ))
                                    }
                                </TableBody>
                            </Table>
                        </TableContainer>
                    </Grid>
                    <Grid item xs></Grid>
                </Grid>
            </Box>
            <Box sx={{ flexGrow: 1, mt: '50px' }}>
                <Grid container spacing={3}>
                    <Grid item xs></Grid>
                    <Grid item xs={10}>
                        <Typography variant='h4' marginBottom='10px' fontFamily='Orbitron' sx={{ float: 'left' }}>
                            CHART:
                        </Typography>
                        <Box sx={{ minWidth: 120, mb: "50px" }}>
                            <FormControl fullWidth>
                                <InputLabel
                                    variant="standard"
                                    htmlFor="uncontrolled-native"
                                    sx={{
                                        "&.Mui-focused": {
                                            color: "#e10600"
                                        }
                                    }}
                                >
                                    Grand Prix
                                </InputLabel>
                                <NativeSelect
                                    value={detailData}
                                    inputProps={{
                                        name: 'grandPrix',
                                        id: 'uncontrolled-native',
                                    }}
                                    onChange={(e) => {
                                        setDetailData(e.target.value);
                                        setDetail(e.target.value);
                                    }
                                    }
                                >
                                    {
                                        arrDetail.map((item) => (
                                            <option
                                                key={item.grandPrix}
                                                value={item.grandPrix}
                                            >
                                                {item.grandPrix}
                                            </option>
                                        ))
                                    }
                                </NativeSelect>
                            </FormControl>
                        </Box>
                        <BarChart chartData={chart}/>
                    </Grid>
                    <Grid item xs></Grid>
                </Grid>
            </Box>
        </>
    )
}
